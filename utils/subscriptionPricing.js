 ...
 */
export async function computeSubscriptionItemPrice(plan, itemsRequested, { express = false } = {}, usage = null) {
  const monthlyItems = plan.monthly_items ?? 20; // fallback
  const overageFee = plan.overageFee ?? 500;     // fallback
  const priorityOverhead = plan.priority_overhead_per_item_ngn ?? 0;

  const usedItems = usage?.items_used ?? 0;
  const remainingAllowance = Math.max(0, monthlyItems - usedItems);

  // Determine included vs overage
  ...
  // Price for included items
  const includedPrice = express ? includedItems * priorityOverhead : 0;

  // Price for overage items
  const overagePrice = overageItems * (overageFee + (express ? priorityOverhead : 0));

  const totalFee = includedPrice + overagePrice;

  // Per-item price (spread total over all items)
  const perItemPrice = totalFee / itemsRequested;
