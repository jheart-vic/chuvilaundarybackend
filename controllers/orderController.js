...
export const createOrder = async (req, res, next) => {
  try {
    ...
    // 5️⃣ Resolve serviceTier (differs by model)
    const serviceTier =
      pricingModel === 'SUBSCRIPTION'
        ? plan?.serviceTier || 'STANDARD' // enforce from plan
    ...
    const expectedReadyAt = computeExpectedReadyAt(
      ...
      plan?.serviceTier || serviceTier, // subscription tier wins, else retail tier
      { express: hasExpress }
    )

    // derive hours from difference
